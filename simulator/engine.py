...
SERVICE_TIME_MULTIPLIERS_PATH = PROJECT_ROOT / "service_time/service_time_multipliers.csv"
...
def _load_service_time_multipliers(path: Path) -> dict[tuple[str, str], float]:
    ...
    if not required.issubset(df.columns):
        print(f"[WARN] {path.name} no contiene columnas {required}, se ignoran multiplicadores.")
        return {}
    df["lane_type"] = df["lane_type"].astype(str).str.strip().str.lower().map(LANE_NAME_NORMALIZATION).fillna(df["lane_type"])
...
_ARRIVAL_FILES = {
    ...
    CustomerProfile.SELF_CHECKOUT_FAN: ARRIVALS_NPZ_DIR / "lambda_self_checkout_fan.npz",
...
def _load_arrival_series(path: Path) -> dict[tuple[DayType, PriorityType, PaymentMethod], tuple[np.ndarray, np.ndarray]]:
    ...
    series: dict[tuple[DayType, PriorityType, PaymentMethod], tuple[np.ndarray, np.ndarray]] = {}
    for idx, key_str in enumerate(keys):
        ...
        series[(day_type, priority, payment)] = (bin_left.copy(), lambda_matrix[idx].astype(np.float32, copy=False))
    return series
...
def _lambda_step_at(seconds: float, times: np.ndarray, lambdas: np.ndarray) -> float:
    ...
    if seconds >= times[-1]: return float(lambdas[-1])  # â† antes devolvÃ­as 0.0
...
def _build_kde_model(support: np.ndarray,
                     probs: np.ndarray,
                     kernel: Optional[str],
...
def load_item_distributions(path: Path = ITEMS_SUMMARY_FILE) -> Dict[Tuple[CustomerProfile, PriorityType, Optional[PaymentMethod], Optional[DayType]], ItemDistributionSpec]:
    store: Dict[Tuple[CustomerProfile, PriorityType, Optional[PaymentMethod], Optional[DayType]], ItemDistributionSpec] = {}
    with path.open(newline="", encoding="utf-8") as fh:
        reader = csv.DictReader(fh)
        for row in reader:
            try:
                ...
                continue
    if not store:
        raise RuntimeError("No se pudieron cargar distribuciones de items desde items_distribution_summary.csv")
    return store
...
def _resolve_item_spec(profile: CustomerProfile,
                       priority: Optional[PriorityType],
                       payment: Optional[PaymentMethod],
                       day_type: Optional[DayType]) -> Optional[ItemDistributionSpec]:
...
# %%
def draw_items(profile,
               priority: Optional[PriorityType] = None,
               payment_method: Optional[PaymentMethod] = None,
               day_type: Optional[DayType] = None,
               rng=RNG_ITEMS) -> int:
    prof = profile if isinstance(profile, CustomerProfile) else _norm_enum(profile, CustomerProfile)
    prio = priority if isinstance(priority, PriorityType) or priority is None else _norm_enum(priority, PriorityType)
...
class _PatienceRuleStore:
    def __init__(self):
        self.rules: Dict[Tuple[Optional[CustomerProfile], PriorityType, Optional[PaymentMethod]], Dict[str,Any]] = {}
    def add(self, prof: Optional[CustomerProfile], prio: PriorityType,
            payment: Optional[PaymentMethod], spec: Dict[str,Any]):
        self.rules[(prof, prio, payment)] = spec
    def find(self, prof: CustomerProfile, prio: PriorityType,
             payment: Optional[PaymentMethod]) -> Optional[Dict[str,Any]]:
        candidates = [
            (prof, prio, payment),
            (prof, prio, None),
...
def _parse_params_str(s: str) -> dict:
    ...
    if s.startswith("(") and s.endswith(")"):
        s = s[1:-1].strip()
    # key=value
    pairs = re.findall(r'([a-zA-Z_Ã¡Ã©Ã­Ã³ÃºÃ±]+)\s*=?\s*([-+]?\d*\.?\d+(?:e[-+]?\d+)?)', s)
    if pairs:
        out = {}
        for k,v in pairs:
            k = k.lower().replace("Ã¡","a").replace("Ã©","e").replace("Ã­","i").replace("Ã³","o").replace("Ãº","u")
            out[k] = float(v)
        # alias
...
def _row_to_spec(row: Dict[str,Any]) -> Dict[str,Any]:
    ...
    p_raw = (row.get("parameters") or row.get("parametros") or row.get("parÃ¡metros") or row.get("parametros_aprox") or "")
...
class PatienceDistributionCSV:
    ...
    def sample(self,
               ...
               priority: PriorityType,
               payment: Optional[PaymentMethod],
               day_type: Optional[DayType] = None) -> float:
...
class PatienceDistributionTable:
    ...
    def _load_entries(path: Path) -> dict:
        path = Path(path)
        if not path.exists():
            raise FileNotFoundError(f"No existe el archivo de distribuciones de paciencia: {path}")
        df = pd.read_csv(path)
        required_cols = {"profile", "priority", "payment_method", "day_type", "method"}
        if not required_cols.issubset(df.columns):
            raise ValueError(f"El archivo {path} no contiene las columnas requeridas {required_cols}")
        for col in ["profile", "priority", "payment_method", "day_type", "method"]:
            df[col] = df[col].astype(str).str.strip().str.lower()
        df["payment_method"] = df["payment_method"].apply(lambda v: _norm_payment(_normalize_payment_value(v)))
        df["day_type"] = df["day_type"].apply(_norm_day_type)
        table = {}
        for (prof, prio, pay, day), grp in df.groupby(["profile", "priority", "payment_method", "day_type"]):
            if (grp["method"] == "param").any():
                ...
                if dist and params:
                    table[(prof, prio, pay, day)] = {"type": "param", "distribution": dist, "params": params}
                    continue
    ...
    def _sample_from_entry(
        ...
        if entry["type"] == "param":
            ...
            x = entry["x"]
            cdf = entry["cdf"]
            if isinstance(x, np.ndarray) and isinstance(cdf, np.ndarray) and x.size >= 2:
                u = self.rng.random()
...
def _priority_request_value(priority: PriorityType) -> int:
    return _PRIORITY_QUEUE_RANK.get(priority, _PRIORITY_QUEUE_RANK[PriorityType.NO_PRIORITY])
...
SERVICE_TIME_CATEGORY_COLUMNS = ["profile", "priority", "payment_method", "lane_type", "day_type"]
...
class ServiceTimeFactorModel:
    ...
    def __init__(self, model_path: Path, multipliers: Optional[dict[tuple[str, str], float]] = None, rng_seed: int = 2025):
        ...
        for col, entry in coeffs.get("categories", {}).items():
            ...
            coeff_map = {str(k).strip().lower(): float(v) for k, v in entry.get("coeffs", {}).items()}
    ...
    def _build_distribution(self, entry: Optional[dict]) -> Optional[_ServiceTimeResidualDistribution]:
        if not entry:
            return None
    ...
    def _build_bucket(self, entry: Optional[dict]) -> _ServiceTimeResidualBucket:
        ...
        return _ServiceTimeResidualBucket(prob_outlier=prob, inlier=inlier, outlier=outlier)
    ...
    def _lookup_bucket(self, profile, priority, payment_method, day_type, lane_type) -> _ServiceTimeResidualBucket:
    ...
    def _category_adjustment(self, column: str, value) -> float:
        ...
        if not info:
            return 0.0
        normalizer = self.CATEGORY_NORMALIZERS.get(column, lambda x: str(x or "").strip().lower())
        norm_value = normalizer(value)
        return float(info["coeffs"].get(norm_value, 0.0))

    def _base_prediction(self, profile, priority, payment_method, day_type, lane_type, items: float) -> float:
    ...
    def expected_value(self, *, profile, priority, payment_method, day_type, lane_type, items: float) -> float:
        base = self._base_prediction(profile, priority, payment_method, day_type, lane_type, items)
        bucket = self._lookup_bucket(profile, priority, payment_method, day_type, lane_type)
...
def _resolve_profit_entry(profile_key: str,
                          priority_key: str,
                          payment_key: str,
                          day_type: DayType,
                          profit_data: dict) -> Optional[dict]:
    candidates = []
    if payment_key:
...
def sample_profit(items: int,
                  ...
                  profit_dict: dict = PROFIT_BETAS) -> float:
    """
    ...
    entry = _resolve_profit_entry(profile_key, priority_key, payment_txt, day_norm, profit_dict)
...
def finalize_customer_profit(customer, *, profit_dict=PROFIT_BETAS, default_day_type=DayType.TYPE_1):
...
def _build_lambda_matrix(combos: list[tuple], day_len: int) -> np.ndarray:
    ...
    for idx, (_, _, _, dist) in enumerate(combos):
        ...
        lam_sec = np.maximum(np.asarray(lam, dtype=LAMBDA_MATRIX_DTYPE), 0.0) / np.float32(60.0)
...
def _simular_dia_periodo(
    ...
    def add(prefix: str, lane_type: LaneType, cantidad: int) -> None:
        ...
        if lane_type is LaneType.SCO:
            ...
            for _ in range(cantidad):
                idx += 1
                lanes.append(CheckoutLane(env, f"{prefix}-{idx}", lane_type, capacity=1))
            return
    ...
    while t < t1:
        ...
        patience = cfg_by_prof[prof].patience_distribution.sample(prof, priority, payment_method, day_type)
